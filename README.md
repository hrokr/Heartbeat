...
Physionet did an excellent job with structuring the recordings. They kept a separate holdout set for validation that was released only after the competition was concluded. The data researchers were give was pre-randomized, and split into five subsets (for ease with k-fold), each with a separate files listing which recordings were normal and abnormal (to facilitate both supervised and unsupervised learning). More information on the datas as well as the results of the top models can be found at the [challenge website](https://physionet.org/content/challenge-2016/1.0.0/).
